...
export function Profile() {
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="text-center space-y-3">
        {user.avatar && (
          <img
            ...
            alt=""
            className="w-20 h-20 rounded-full mx-auto ring-2 ring-cyan-500/50"
          />
        )}
        <h1 className="text-3xl font-bold text-white">{user.username}</h1>
        <div className="text-lg font-mono text-cyan-400">{totalPoints} points</div>
      </div>

      {/* Stats grid */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
        {statCards.map(card => (
          <div key={card.label} className="bg-gray-900/50 border border-cyan-500/20 rounded-xl p-5 text-center backdrop-blur-sm">
      ...
      {/* Win rate */}
      <div className="max-w-md mx-auto bg-gray-900/50 border border-cyan-500/20 rounded-xl p-6 text-center backdrop-blur-sm">
